(function()
{
	...
	document.addEventListener('DOMContentLoaded', function()
	{
		...
		document.getElementById('insertInserttagsGhsvsCode').addEventListener('click', function()
		{
			...
			// e.g <code class=code-filename>...</code>. e.g. <kbd class=kbd-klickweg>...</kbd>
			else if(
				formData.selectTag.indexOf('||') > 0
				&& (parts = formData.selectTag.split('||'))
				&& parts.length >= 2
			){
				if (parts[0] === 'abbr')
				{
					startTag = `<${parts[0]}>${parts[1]}`;
					endTag = `</${parts[0]}>`;
				}
				// e.g. <span class=code-eigenname>Joomla</span>
				else if(
					parts.length === 3
					&& parts[0] === 'span'
				){
					startTag = `<${parts[0]} class="${parts[1]}">${parts[2]}`;
					endTag = `</${parts[0]}>`;
				}
				else
				{
					startTag = `<${parts[0]} class="${parts[0]}-${parts[1]}">`;
	
					if (parts[2])
					{
						startTag = startTag + parts[2];
					}
